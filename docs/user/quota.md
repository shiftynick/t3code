...
Quota shows how much of your Claude and Codex subscription is left on the machine
running the T3 Code server. It is separate from the Usage page, which estimates
API-equivalent token cost from local session history.
...
Codex CLI signed in with ChatGPT. Cursor, Grok, and OpenCode are not included.
